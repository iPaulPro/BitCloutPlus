...
const getUserMetadata = (publicKey) => {
  ...
  return fetch(`${getBaseUrl()}/api/v0/get-user-metadata/${publicKey}`)
    .then(res => res.json())
}
...
const submitPost = (
  ...
  if (!extraData['Node']) {
    extraData['Node'] = window.location.hostname === 'node.deso.org' ? '1' : '2'
  }
...
const getNftsForUser = (publicKey, isForSale) => {
  ...
  return fetch(`${getBaseUrl()}/api/v0/get-nfts-for-user`, request)
    .then(res => res.json())
...
const burnNft = (publicKey, nftPostHashHex, serialNumber) => {
  ...
  return fetch(`${getBaseUrl()}/api/v0/burn-nft`, request)
    .then(res => res.json())
}
...
const acceptTransferNft = (publicKey, nftPostHashHex, serialNumber) => {
  ...
  return fetch(`${getBaseUrl()}/api/v0/accept-nft-transfer`, request)
    .then(res => res.json())
}
