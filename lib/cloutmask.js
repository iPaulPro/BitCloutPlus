...
function switchToFirstAccount(identityUsers = getIdentityUsers()) {
  if (!identityUsers) return
...
const removePublicKeyFromIdentityUsers = (key) => {
  ...
  if (key === getLoggedInPublicKey()) switchToFirstAccount(identityUsers)
}
...
const removeAllMaskedUsers = () => {
  ...
  if (loggedInAsMaskedUser) switchToFirstAccount(identityUsers)
}
