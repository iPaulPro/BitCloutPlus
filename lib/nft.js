...
const createTransferButton = (publicKey, postHashHex) => {
  const restoreButton = (button) => {
    button.disabled = false
  ...
  transferButton.onclick = () => {
    ...
    const textIsPublicKey = text.startsWith(DESO_PUBLIC_KEY_PREFIX)
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        return textIsPublicKey ? getProfileByPublicKey(text) : getProfileByUsername(text)
      } else {
        restoreButton(transferButton)
      }
    }).then(profile => {
