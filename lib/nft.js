...
function createSecondaryButton(text) {
  const button = document.createElement('button')
  button.type = 'button'
  button.className = 'btn btn-warning font-weight-bold br-8px fs-13px'
  button.innerText = text
  return button
}
  ...
  getPendingNftTransfers(publicKey).then(pendingNfts => {
    ...
    pendingNfts.forEach(value => {
      ...
      nftEntries.forEach(nft => {
        ...
        const returnButton = createSecondaryButton('Return')
        const postHashHex = postEntry['PostHashHex']
        const lastOwnerPublicKey = nft['LastOwnerPublicKeyBase58Check']
        returnButton.onclick = () => window.location.href = `/nft/${postHashHex}/transfer?publicKey=${lastOwnerPublicKey}`

        const acceptButton = createAcceptButton(publicKey, postEntry, serialNumber, returnButton)

        const buttonElements = [serialNumberSpacer, returnButton, acceptButton]
