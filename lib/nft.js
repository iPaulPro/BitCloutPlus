...
const onSerialNumberSelected = () => {
  ...
  console.log(`onSerialNumberSelected ${serialNumber}, owned entries = ${JSON.stringify(transferNftOwnedEntries)}`)
  ...
  if (encryptedUnlockableText) {
    console.log(`Decrypting unlockable text from previous owner`)
  ...
  if (hasUnlockable) {
    ...
    unlockableTextDiv.onclick = () => showUnlockableTextDialog().then((res) => {
      console.log(`Got unlockable text dialog response: ${JSON.stringify(res)}`)
      if (res.isConfirmed) {
...
const showUnlockableTextDialog = () => {
  console.log(`show unlockable text dialog: inputValue = ${transferNftUnlockableText}`)
...
const onNftTransferUnlockableDecrypted = (unlockableText) => {
  console.log(`Decrypted unlockable text: ${unlockableText}`)
...
const createTransferButton = (senderPublicKey, postHashHex) => {
  ...
  transferButton.onclick = () => {
    ...
    const hasUnlockable = transferNftPostEntry['HasUnlockable']
    console.log(`on transfer button click hasUnlockable: ${hasUnlockable}`)

    if (hasUnlockable) {
      getRecipientProfileFromInput().then(profile => {
        const recipientPublicKey = profile['PublicKeyBase58Check']
        console.log(`Got recipient public key: ${recipientPublicKey}`)
